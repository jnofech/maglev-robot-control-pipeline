...
p.FaceColor = p.EdgeColor;
p.FaceAlpha = 0.2*p.EdgeAlpha;

% Apply PV pairs
for ii = 1:2:numel(PVs)
    if ismember(lower(PVs{ii}),['edgecolor','color'])
        if numel(PVs{ii+1})==4
            set_alpha = PVs{ii+1}(4);
            PVs{ii+1} = PVs{ii+1}(1:3);
            set(p,'facealpha',set_alpha*0.2);
            set(p,'edgealpha',set_alpha);
        end
        set(p,'facecolor',PVs{ii+1});
        set(p,'edgecolor',PVs{ii+1});
    elseif ismember(lower(PVs{ii}),['edgealpha','alpha'])
        set(p,'facealpha',PVs{ii+1}*0.2);
        set(p,'edgealpha',PVs{ii+1});
    else
