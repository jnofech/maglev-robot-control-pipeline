function [t_fits, y_expofit, y_sinefit, y_final_expo, y_final_sine, y_final_sine_firstv0, final_selection] = xy_find_finals(t,y,verbose,extrap_mode)
%XY_FIND_FINALS takes a segment of a position-vs-time signal and finds the
%   location where the position converges.
%
%   PARAMS:
%   -------
%   t               : time
%   y               : position 
%   verbose (logical)   : toggles diagnostic outputs
    ...
    time_tooshort_ms = 100;                 % Threshold for indicating relatively short recording
    score_diff_threshold = 0.1;             % Threshold for clear differences in adjusted R-Squared score (additive)
    score_quot_threshold = 5;               % Threshold for clear differences in adjusted R-Squared score (multiplicative)
    ...
    if extrap_mode=="skip"
        ...
        y_final_expo = nan;
        y_final_sine = nan;
        y_final_sine_firstv0 = nan;
        return
    end
    if (abs(max(y) - min(y)) < barelychanges_threshold_micron / 1000) && (t(end)-t(1)<time_tooshort_ms)
        if extrap_mode=="strict"
            if verbose disp("FINAL SELECTION REPORT: Segment too short (space AND time-wise); returning NaN and aborting"); end
            final_selection = nan;
        elseif extrap_mode=="lax"
            if verbose disp("FINAL SELECTION REPORT: Segment too short (space AND time-wise); returning final known position and aborting"); end
            final_selection = y(end);
        else
            error("xy_find_finals() : Invalid `extrap_mode`")
        end
        t_fits = t;
        y_expofit = nan*y;
        y_sinefit = nan*y;
    ...
    for n_points_extra = 0:10     % Additional, if needed due to GOF errors (!!!!INCREMENT AUTOMATICALLY IF NEEDED!!!!)
        ...
        try
            ...
            fprintf(1,'ERROR DETECTED! Message: \n%s\n',e.message);
            if extrap_mode=="strict"
                if true disp("FINAL SELECTION REPORT: Error occurred; returning NaN and aborting"); end
                final_selection = nan;
            elseif extrap_mode=="lax"
                if true disp("FINAL SELECTION REPORT: Error occurred; returning final known position and aborting"); end
                final_selection = y(end);
            else
                error("xy_find_finals() : Invalid `extrap_mode`")
            end
        ...
        if abs(exp_gof.adjrsquare)+abs(sine_gof.adjrsquare) > 1e10   % If adjusted RSquare indicates an error:
            ...
            if verbose disp("Error detected; retrying."); end
        ...
        if n_points_extra~=0 && failed_to_generate_GOF==false
            if verbose disp("Successfully completed fits after "+string(n_points_extra)+" attempts"); end
        elseif failed_to_generate_GOF==true
            if verbose disp("Failed to resolve erroneous GOF statistics within "+string(n_points_extra)+" attempts"); end
        end
    ...
    if (~sine_always_increasing && sine_gof.adjrsquare > 0.8)
        if verbose disp("OBVIOUS OSCILLATION DETECTED"); end
    ...
    if relative_accel_expo < straightline_threshold && relative_accel_sine < straightline_threshold
        if verbose disp("FINAL SELECTION PRE-REPORT: Looks like a straight line"); end
        exp_gof.adjrsquare = 0;
        sine_gof.adjrsquare = 0;
    elseif relative_accel_expo < straightline_threshold
        if verbose disp("FINAL SELECTION PRE-REPORT: Expo fit is a straight line"); end
        exp_gof.adjrsquare = 0;

        % "Strict" mode
        if extrap_mode=="strict"
            if verbose disp("WARNING                   : Expo fit being a straight line probably means the sine fit doesn't show decelerating!"); end
            sine_gof.adjrsquare = 0;
        % "Generous" mode (may lead to unreliable data)
        elseif extrap_mode=="lax"
            if sign(accel_sine(end)) ~= -endpoint_direction
                if verbose disp("                  ADDENDUM: Sine fit is accelerating the wrong way"); end
                sine_gof.adjrsquare = 0;
            end
        else
            error("xy_find_finals() : Invalid `extrap_mode`")
    ...
    elseif relative_accel_sine < straightline_threshold
        if verbose disp("FINAL SELECTION PRE-REPORT: Sine fit is a straight line"); end
        sine_gof.adjrsquare = 0;
    end
    % Conditionals:
    ...
    if abs(max(y) - min(y)) < barelychanges_threshold_micron/2 / 1000
        if verbose disp("FINAL SELECTION REPORT: Signal barely changes; returning endpoint as final value"); end
    ...
    %     if verbose disp("FINAL SELECTION REPORT: Looks like a straight line")
    %     final_selection = nan;
    ...
    %         if verbose disp("FINAL SELECTION REPORT: Only sine fit doesn't look like a straight line!")
    ...
    %         if verbose disp("FINAL SELECTION REPORT: Only expo fit doesn't look like a straight line!")
    ...
    elseif strong_oscillation==true
        if verbose disp("FINAL SELECTION REPORT: Obvious oscillation detected"); end
        final_selection_options = [final_resting_position_sine, final_resting_position_sine_firststop];
        final_selection = choose_from(final_selection_options, y(end), endpoint_direction);
    %   3. If velocity of last ~100ms is close to 0, just take final position
    %   as endpoint
    elseif max(t) > 110 && 1000*(max(y(max(1,end-100):end))-min(y(max(1,end-100):end))) < barelychanges_threshold_micron/5
        if verbose disp("FINAL SELECTION REPORT: Velocity near end is close to 0"); end
        final_selection = final_resting_position_finalpoint;
    ...
    elseif failed_to_generate_GOF
        if verbose disp("FINAL SELECTION REPORT: At least one GOF failed to generate"); end
    ...
    elseif abs(compare_score_diff) > score_diff_threshold || (compare_score_quot > score_quot_threshold)
        if compare_score_diff > 0
            if verbose disp("FINAL SELECTION REPORT: Sine fit is significantly better!"); end
            % Sine fit wins! Pick from sine fit results
            final_selection_options = [final_resting_position_sine, final_resting_position_sine_firststop];
            final_selection = choose_from(final_selection_options, y(end), endpoint_direction);
        else
            if verbose disp("FINAL SELECTION REPORT: Expo fit is significantly better!"); end
            % Expo fit wins! Pick from expo fit results
            final_selection = final_resting_position_exp;
        end
    ...
    elseif exp_gof.adjrsquare > 0.99 && sine_gof.adjrsquare > 0.99
        if verbose disp("FINAL SELECTION REPORT: Both fits are excellent!"); end
    ...
    elseif exp_gof.adjrsquare > 0.99 || sine_gof.adjrsquare > 0.99
        if exp_gof.adjrsquare > 0.99
            if verbose disp("FINAL SELECTION REPORT: Expo fit is excellent!"); end
            final_selection = final_resting_position_exp;
        else
            if verbose disp("FINAL SELECTION REPORT: Sine fit is excellent!"); end
            final_selection_options = [final_resting_position_sine, final_resting_position_sine_firststop];
            final_selection = choose_from(final_selection_options, y(end), endpoint_direction);
        end
    ...
    elseif exp_gof.adjrsquare > 0.75 && sine_gof.adjrsquare > 0.75
        if verbose disp("FINAL SELECTION REPORT: Both fits are... reasonably good i guess?"); end
        final_selection_options = [final_resting_position_exp, final_resting_position_sine_firststop];
        final_selection = choose_from(final_selection_options, y(end), endpoint_direction);
    %  99. else, the segment is probably a straight line and thus unusable. Discard! 
    else
        if verbose disp("FINAL SELECTION REPORT: Fits failed; disregard this segment"); end
        final_selection = nan;
    % (MISC)
    ...
    if abs(final_selection - y(end)) > abs(y(end) - y(1))*2
        if verbose disp("FINAL SELECTION REPORT ADDENDUM: Best option is still too far; disregard this segment"); end
        final_selection = nan;
    end
    ...
    % Finalize outputs
    t_fits = t_extrap_ms;
    y_expofit = exp_fit(t_extrap) * y_scale + y_initial;
    y_sinefit = sine_fit(t_extrap) * y_scale + y_initial;
    y_final_expo = final_resting_position_exp;
    y_final_sine = final_resting_position_sine;
    y_final_sine_firstv0 = final_resting_position_sine_firststop;
